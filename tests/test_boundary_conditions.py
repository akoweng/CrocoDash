...
def test_setup_initial_condition():
    ...
    bc_obj.setup_initial_condition(hgrid, vgrid, Path(glorys_path)/"ic_unprocessed.nc", ocean_varnames)
...
def test_setup_ocean_state_boundaries():
    ...
    bc_obj.setup_ocean_state_boundaries(hgrid, vgrid,"2020-01-01", glorys_path, ocean_varnames, boundaries = ["south", "north", "west", "east"])
    assert bc_obj.temp_storage == ".crr_bcc_temp"
...
def test_setup_boundary_tides():
    ...
    bc_obj.setup_boundary_tides(hgrid, "2020-01-01",tidal_constituents, Path("/glade/u/home/manishrv/manish_scratch_symlink/inputs_rm6/tidal_data"),Path("tpxo9.v1.nc"))
